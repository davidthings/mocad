...
module smallMotorBevelGearbox( p = 0, a = 0, info = false ) {
  moMoveOriginTo( smallMotorBevelGearboxPosition( p, a ) )  {
   ...
   moMoveTo( actMotorMountPlanetaryA( 1 ) )
    ...
    if ( info == true ) {
      for ( pi = [ 0 : 3 ] )
        moMoveTo( smallMotorBevelGearboxPosition( pi, a ) )
          moPosition( pi );
    }
  }
  if ( info == true )
...
function smallMotorBevelGearboxPosition( p, a ) = ( p == 0 ) ? [ [ 0, 0, 0 ], [ 0, 0, 1 ], 0 ] :
...
echo( actBevelGear32Pitch16Tooth4Position( 2 ) );
echo( moInvertPosition( actBevelGear32Pitch16Tooth4Position( 2 ) ) );

a = moSuperSmoothAnimationCycle( 0, 0.7, 90 );

smallMotorBevelGearbox( a = a, info = true, $name=true );
