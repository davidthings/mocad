...
module actHexShaft12HubAdapter( p = 0, info = false ) {
  moPresent( positions, p, info )
    color( Steel )
      moFlip()
        import("./stl/actobotics_hex_hub_wheel_adapter.stl");
}

function actHexShaft12HubAdapterPosition( p = 0 ) = positions[ p ];

moMoveOriginTo( actHexShaft12HubAdapterPosition( 0 ) )
  actHexShaft12HubAdapter( 0, info = true, $name = true );
